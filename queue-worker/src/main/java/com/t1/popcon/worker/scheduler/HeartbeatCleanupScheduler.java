 ...
 * - 주기: queue.heartbeat-cleanup-interval-ms (기본 10초)
 * - 동작: Redis SCAN으로 활성 phase 탐색 → 만료된 heartbeat 사용자 정리
 * - 분산 락: 멀티 인스턴스 환경에서 중복 실행 방지 (락 획득 실패 시 즉시 스킵)
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class HeartbeatCleanupScheduler {
    ...
    public void cleanupExpiredHeartbeats() {
        ...
        try {
            ...
            log.debug("[HeartbeatCleanupScheduler] 활성 phase 수: {}", activePhases.size());

            for (PhaseKey phase : activePhases) {
                try {
                    workerService.cleanupExpiredHeartbeats(phase.phaseType(), phase.phaseId());
                } catch (Exception e) {
                    log.error("[HeartbeatCleanupScheduler] 정리 실패 - phase={}", phase, e);
                }
            }
        } finally {
